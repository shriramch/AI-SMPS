...
Turn OthelloGame::getTurn() { return turn; }
...
void OthelloGame::makeMove(const Move &move) { board.makeMove(turn, move); }
  ...
  do {
    ...
    if (!board.getValidMoves(turn).empty()) {
      ...
      try {
        ...
        throw BotInvalidMoveException(player, e.move, board.getBlackCount(),
                                      board.getRedCount());
      }
    } else {
...
bool OthelloGame::isGameOver() {
  ...
  return (board.getValidMoves(turn).empty() &&
          board.getValidMoves(other(turn)).empty());
}
...
static Move launchEnvironment(OthelloPlayer &player, OthelloBoard &board) {
  ...
  s = pthread_create(&t_id, &attr, (start_routine)createEnvironment,
                     &environ) != 0;
  if (s != 0) {
    errno = s;
  ...
  case EFLAGS_TIMEOUT:
    throw TimeoutException(environ.player, board.getBlackCount(),
                           board.getRedCount());
    break;
